class Forecast
  ...
  def get_current_weather(data)
    { datetime: Time.at.getlocal(data[:dt]),
      sunrise: Time.at.getlocal(data[:sunrise]),
      sunset: Time.at.getlocal(data[:sunset]),
  ...
  def get_hourly_weather(hourly_data)
    hourly_data.drop(1).take(8).map do |hour_data|
      {
        time: Time.at.getlocal(hour_data[:dt]).strftime('%I:%M:%S'),
  ...
  def get_daily_weather(daily_data)
    daily_data.drop(1).take(5).map do |day_data|
      { date: Time.at.getlocal(day_data[:dt]).strftime('%Y-%m-%d'),
        sunrise: Time.at.getlocal(day_data[:sunrise]),
        sunset: Time.at.getlocal(day_data[:sunset]),
